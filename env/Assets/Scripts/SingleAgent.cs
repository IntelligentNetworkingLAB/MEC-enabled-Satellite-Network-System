    ...
    float GetReward()
    {
        ...
        if (isKind) // cube
        {
            ...
            for (int i = 0; i < iTaskSize; ++i)
            {
                ...
                if (vTaskAssinged[i]) // CubeSats
                {
                    var bandwidth = 2e+3f * (1f / 20f);    // MHz
                    var power = 1e+2f * (1f / 20f);         // Gigacycles/sec
                    var SNR = 1f;
                    Ttran = vTaskMemory[i] / (bandwidth * Mathf.Log(1 + SNR, 2));
                    Ptran = Xtran_c * bandwidth;
                ...
                else // LMS
                {
                    ...
                    var power = 8e+2f * (1f / 100f);         // Gigacycles/sec
                    var SNR = 1f;
                    Ttran = vTaskMemory[i] / (bandwidth * Mathf.Log(1 + SNR, 2));
